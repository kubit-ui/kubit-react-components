...
test('it should remove nodes from the original code', async () => {
    ...
    const codeWithoutImportDeclarations = assembleUpdatedCode(code, commentAndImportsToRemoveFromCode);
...
test('it should inject the generated code at the correct location', async () => {
    ...
    const codeWithoutImportDeclarations = assembleUpdatedCode(code, commentAndImportsToRemoveFromCode, `import generated from "generated";`, '"use strict";'.length);
    const result = await format(codeWithoutImportDeclarations, {
        parser: 'babel',
    });
    expect(result).toEqual(`"use strict";
