...
exports.INTERNAL_STATE_SYMBOL = void 0;
exports.createFlattenedArgs = createFlattenedArgs;
exports.createFlattenedArgTypes = createFlattenedArgTypes;
exports.expandObject = expandObject;
const general_1 = require("./general");
/** @internal */
exports.INTERNAL_STATE_SYMBOL = Symbol("deepControlsInternalState");
...
function createFlattenedArgs(context) {
    return flattenObjectRecursively(context.initialArgs, "", {
        ...
        context.parameters.deepControls?.[exports.INTERNAL_STATE_SYMBOL]?.userDefinedArgTypes ?? {},
    });
}
function flattenObjectRecursively(nestedObject, pathToParent, context) {
    ...
    Object.entries(nestedObject).forEach(([key, value]) => {
        ...
        const shouldFlatten = (0, general_1.isPojo)(value) && !context.userDefinedArgTypes[key] && Object.keys(value).length;
        if (!shouldFlatten) {
 ...
 */
function userAlreadyDefinedArgTypeForParentOfThisPath(argPath, userDefinedArgTypes) {
...
function createFlattenedArgTypes(context) {
    const internalState = context.parameters.deepControls?.[exports.INTERNAL_STATE_SYMBOL];
    ...
    Object.defineProperty(context.parameters.deepControls, exports.INTERNAL_STATE_SYMBOL, {
    ...
    */
    ...
    for (const [argPath, argValue] of Object.entries(flatInitialArgs)) {
        const generatedArgType = createFlattenedValueArgType(argPath, argValue, controlMatcherEntries, userDefinedArgTypes);
...
function createFlattenedValueArgType(argPath, argValue, controlMatcherEntries, userDefinedArgTypes) {
    ...
    if (Array.isArray(argValue) || ((0, general_1.isPojo)(argValue) && !Object.keys(argValue).length)) {
...
function mergeArgTypes(target, overrides) {
    if (!target) {
        return overrides;
    }
    if (!overrides) {
    ...
    const flatOverrideEntries = Object.entries(flattenObjectRecursively(overrides, "", {
        flatObjectOut: {},
        userDefinedArgTypes: {},
    }));
    ...
    for (const [overridePath, overrideValue] of flatOverrideEntries) {
        (0, general_1.setProperty)(target, overridePath, overrideValue);
    }
    return target;
}
function getArgTypeFromControlMatchers({ argPath, controlMatcherEntries, }) {
    ...
    for (const [controlType, matcherRegex] of controlMatcherEntries) {
        ...
        if (matcherRegex.test(lastSegment)) {
            return {
                name: argPath,
                control: { type: controlType }, // ControlType not exported
            };
        }
    }
}
 ...
 */
function getUserDefinedArgTypes({ argTypes = {}, parameters, }) {
    if (!parameters.docs) {
 ...
 */
function isArgTypeLikelyGeneratedByDocs(argName, argType) {
