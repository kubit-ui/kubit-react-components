...
describe('Date functions', () => {
  ...
  it('should return a new date with the specified number of months added', () => {
    ...
    expect(formatDate(getAddMonths(date1, 5), 'dd-MM-yyyy')).toEqual(
      expectedDate,
    );
  });
  ...
  it('should return a new date with the specified number of months subtracted', () => {
    ...
    expect(formatDate(getSubMonths(date1, 5), 'dd-MM-yyyy')).toEqual(
      expectedDate,
    );
  });
  ...
  it('should return a new date with the specified number of years subtracted', () => {
    ...
    expect(formatDate(getSubYears(date1, 5), 'dd-MM-yyyy')).toEqual(
      expectedDate,
    );
  });
  ...
  it('should return an array of all weekday names', () => {
    ...
    expect(getAllWeekdayNames('long', false).sort()).toEqual(
      expectedWeekdayNames.sort(),
    );
  });
});
