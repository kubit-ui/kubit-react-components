...
export const keyLeftMove = ({
  ...
  return (previous) => {
    if (isSameYear(maxDate, minDate)) {
      return previous === minDate.getMonth()
        ? maxDate.getMonth()
        : previous - 1;
    }
...
export const keyRightMove = ({
  ...
  return (previous) => {
    if (isSameYear(maxDate, minDate)) {
      return previous === maxDate.getMonth()
        ? minDate.getMonth()
        : previous + 1;
    }
    if (isSameYear(maxDate, currentDate)) {
...
export const keyUpMove = ({
  ...
  return (previous) => {
    ...
    if (isSameYear(maxDate, currentDate)) {
      ...
      return previous <= NUM_DAYS_IN_ROW
        ? FIRST_MONTH
        : previous - NUM_DAYS_IN_ROW;
    }
    if (previous === FIRST_MONTH) {
...
export const keyDownMove = ({
  ...
  return (previous) => {
    ...
    return previous >= DAYS_UNTIL_LAST_ROW
      ? LAST_MONTH
      : previous + NUM_DAYS_IN_ROW;
  };
};
