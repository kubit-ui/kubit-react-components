...
import { useDataTableStickyColumns } from '../../hooks/useDataTableStickyColumns';

describe('useDataTableStickyColumns', () => {
  ...
  beforeEach(() => {
    ...
    tableCell.setAttribute('data-sticky', '');
  ...
  it('should do nothing when there is not scrollable container', () => {
    ...
    renderHook(() => useDataTableStickyColumns({ ref: wrapperWithoutScrollableContainer }));
  ...
  it('When there is horizontal scroll, right style for sticky columns should be set', () => {
    ...
    renderHook(() => useDataTableStickyColumns({ ref }));
  ...
  it('When the right position of the sticky columns have been set and there is no horizontal scroll the dividers width is not set', () => {
    // Simulate offset width for the table row
    Object.defineProperty(tableRow, 'offsetWidth', {
      value: 200,
    });
