  ...
  useEffect(() => {
    ...
    if (scrollableContainer instanceof HTMLElement) {
      const handleElementResize = (element: HTMLElement) => {
        ...
        const stickyLeft =
          element.querySelectorAll('[data-sticky="left"]').length > 0;
        // Calc shadow z-index when sticky left
        let shadowZIndex = 0;
        // Calc divider width when !sticky left
        let dividerWidth = 0;
        if (stickyLeft) {
          const leftBoxShadowContainer = ref.current?.querySelector(
            '[data-datatable-left-shadow]',
          );
          if (leftBoxShadowContainer) {
            const shadowContainerStyle = window.getComputedStyle(
              leftBoxShadowContainer,
            );
            const _shadowZIndex =
              shadowContainerStyle.getPropertyValue('z-index');
            if (_shadowZIndex) {
        ...
        dividers.forEach((divider) => {
          if (divider instanceof HTMLElement) {
            if (stickyLeft) {
              ...
              divider.style.zIndex = _hasHorizontalScroll
                ? `${shadowZIndex + 1}`
                : '';
            } else {
              divider.style.width = _hasHorizontalScroll
                ? `${dividerWidth}px`
                : '';
            }
          }
