...
describe('useScrollDetectionWithAutoFocus', () => {
  ...
  it('Should return hasScroll false if it does not have scroll', () => {
    const { result } = renderHook(() =>
      useScrollDetectionWithAutoFocus({ parentElementRef }),
    );
  ...
  it('should return hasScroll false when the element container has scroll but disabled has been configured', () => {
    Object.defineProperty(element, 'scrollHeight', {
      value: 200,
    });
    Object.defineProperty(element, 'clientHeight', {
    ...
    const { result } = renderHook(() =>
      useScrollDetectionWithAutoFocus({ disabled: true, parentElementRef }),
    );
    act(() => {
      result.current.handleScrollDetection(element);
    });
  ...
  it('When it has scroll and document.active element is null, it should focus on the element with scroll when opening', () => {
    ...
    const mockFocus = vi.spyOn(element, 'focus');
